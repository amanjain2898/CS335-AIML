	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for k in range(trainingLabels[i]):
					first = k
					second = trainingLabels[i]

					if(self.weights[frozenset([first,second])]*trainingData[i] <= 0):
						self.weights[frozenset([first,second])] += trainingData[i]

				for k in range(trainingLabels[i]+1,len(self.legalLabels)):
					first = trainingLabels[i]
					second = k
