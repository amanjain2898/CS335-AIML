...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(1,49):
        for j in range(1,49):
            ...
            if(getPixelVal(i,j+1) - getPixelVal(i,j-1) != 0):
                vedge += 1
            if(getPixelVal(i-1,j) - getPixelVal(i+1,j) != 0):
                hedge += 1
            ...
            if(getPixelVal(i,j) == 1):
                tot += 1
                if(getPixelVal(i,j) != getPixelVal(i-1,j-1)):
                    count = count+1;
                if(getPixelVal(i,j) != getPixelVal(i-1,j)):
                    count = count+1;
                if(getPixelVal(i,j) != getPixelVal(i-1,j+1)):
                    count = count+1;
                if(getPixelVal(i,j) != getPixelVal(i,j-1)):
                    count = count+1;
                if(getPixelVal(i,j) != getPixelVal(i,j+1)):
                    count = count+1;
                if(getPixelVal(i,j) != getPixelVal(i+1,j-1)):
