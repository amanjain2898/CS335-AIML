...
def generate2optNeighbours(tour):
	...
	for i in range(1,len(tour)-1):
		for j in range(i+1,len(tour)):
			if not (i == 1 and j == len(tour)-1):
				a = tour[i:j+1]
				a.reverse()
				tour1 = tour[0:i] + a + tour[j+1:len(tour)]
				if tour1 not in all_possible_neighbours:
					all_possible_neighbours.append(tour1)
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    while 1:
    	all_neighbours = getNeighbours(minTour)
    	min_len = getTourLength(minTour)
    	len1 = min_len
    	flag = 1
    	for i in range(len(all_neighbours)):
    		length = getTourLength(all_neighbours[i])
    		if length < min_len:
    			flag = 0
    			min_len = length
    			minTour = all_neighbours[i]
    	tourLengthList.append(min_len)
    	# print("--iter--")
...
def nearestNeighbourTour(initial_city):
	...
	for i in range(1,cities+1):
		if not i==initial_city:
			rem_cities.append(i)

	for i in range(1,cities):
		min1 = getDistance(nodeDict[tour[i-1]],nodeDict[rem_cities[0]])
		init = rem_cities[0]
		# print("init",init)
		# print("tour",tour[i-1])
		for j in range(1,len(rem_cities)):
			distance = getDistance(nodeDict[tour[i-1]],nodeDict[rem_cities[j]])
			if distance < min1:
				min1 = distance
				init = rem_cities[j]
...
def dfs(mst,initial_city,visited,final_list):
	final_list.append(initial_city)
	visited[initial_city] = 1
	# print(initial_city)

	for (v1,v2) in mst:
		if v1 == initial_city and visited[v2] == 0:
			dfs(mst,v2,visited,final_list)
		if v2 == initial_city and visited[v1] == 0:
			dfs(mst,v1,visited,final_list)
...
def finalOrder(mst, initial_city):
    ...
    visited[0] = 1
    # print(len(visited))
    dfs(mst,initial_city,visited,final_list)
    fin_order = final_list
    # print(final_list)
